    ...
    for idx, folder_name in enumerate(os.listdir(path)):
        ...
        for index, image_name in enumerate(os.listdir(folder_path)):
            ...
            if index >= 1300:   
                ...
                Y[idx * 10 + index-1300] = data
                pixels = image.load()
                data = []
                for i in range(28):
                    for j in range(28):
                        data.append(0 if pixels[i,j] == 0 else 1)
                X[idx * 10 + index-1300] = data
    return X, Y
...
def stochastic_gradient_descent(input_data, result, w_hidden, b_hidden, w_output, b_output, do_dropout, h, r, dropout_percent, lr):
...
def gradient_descent(input_data, result, w_hidden, b_hidden, w_output, b_output, do_dropout, h, r, dropout_percent, lr):
     # forward propagation
...
def main():
    ...
    # training NN
    for i in range(n):
        # stochastic_gradient_descent(input_data, result, w_hidden, b_hidden, w_output, b_output, do_dropout, h, r, dropout_percent, lr)
        gradient_descent(input_data, result, w_hidden, b_hidden, w_output, b_output, do_dropout, h, r, dropout_percent, lr)
